...
- スコアやグラフ用の数値は、期待リターン、目標株価、上昇確率、売買推奨度を表すものではない。
- スコアは、Evidenceの材料方向、直接性、鮮度、影響度、不確実性、価格反応をUIで素早く確認するための材料要約である。
- 数値が高い/低い場合でも、売買行動ではなく「どの材料が市場でどう解釈されやすいか」を説明する。
- `claim_strength` はAgent主張の根拠の強さであり、銘柄の魅力度や投資妙味ではない。
