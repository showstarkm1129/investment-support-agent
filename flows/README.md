...
- `agents/`: role, tone, prohibitions, and decision boundaries for each agent.
- `flows/`: when each agent or connector is called.
- `contracts/`: what each stage must read and write.
...
4. Agents write outputs that match the schemas in `contracts/`.
